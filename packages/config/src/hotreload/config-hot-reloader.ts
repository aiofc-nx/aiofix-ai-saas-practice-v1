...
export class ConfigHotReloader extends EventEmitter {
  ...
  private readonly pathWatchers: Map<
    string,
    Set<(event: IConfigChangeEvent) => void>
  > = new Map();
  private readonly configHistory: Map<string, IConfigHistory[]> = new Map();
  private readonly pendingUpdates: Map<string, IConfigUpdateRequest> =
    new Map();

  private initialized = false;
  private batchUpdateTimer: ReturnType<typeof globalThis.setTimeout> | null =
    null;
  ...
  async triggerUpdate(
    path: string,
    value: unknown,
    options: Partial<IConfigUpdateRequest> = {},
  ): Promise<void> {
   ...
   */
  async rollback(path: string, versions: number = 1): Promise<void> {
   ...
   * @returns 历史记录列表
   */
  getHistory(path: string, limit: number = 10): IConfigHistory[] {
    const history = this.configHistory.get(path) || [];
  ...
  cleanupHistory(path?: string, keepVersions?: number): void {
    const maxVersions =
      keepVersions || this.config.rollback?.maxHistoryVersions || 10;
  ...
  private async handleFileChange(filePath: string): Promise<void> {
    try {
      ...
      await this.debounce(
        `file-${filePath}`,
        this.config.fileWatch!.debounceDelay,
      );
    ...
    try {
      ...
      if (request.validate) {
        ...
        if (!validationResult.valid) {
          throw new Error(
            `配置更新验证失败: ${validationResult.errors.join(', ')}`,
          );
        }
      }
  ...
  private validateUpdatePermission(request: IConfigUpdateRequest): void {
    ...
    if (
      ...
      !allowedSources.includes(request.source)
    ) {
    ...
    return new Promise((resolve) => {
      ...
      (
        this as unknown as Record<
          ...
          ReturnType<typeof globalThis.setTimeout>
        >
      )[`debounce_${key}`] = globalThis.setTimeout(() => {
        delete (
          this as unknown as Record<
            string,
            ReturnType<typeof globalThis.setTimeout>
 ...
 */
export async function createConfigManagerWithHotReload(
  hotReloadConfig?: Partial<IHotReloadConfig>,
): Promise<{
  configManager: IConfigManager;
  ...
  const hotReloader = await createConfigHotReloader(
    configManager,
    hotReloadConfig,
  );
