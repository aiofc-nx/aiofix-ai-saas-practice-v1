  ...
  implements IConfigManager
{
  ...
  > = new Map();
  private readonly changeListeners: Map<
    string,
  ...
  async set<T = unknown>(path: string, value: T): Promise<void> {
    ...
    try {
      ...
      if (this.options.enableValidation) {
        const validationResult = await this.validateConfigUpdate(
          path,
          oldValue,
          value,
        );
  ...
  getStatistics(): IConfigStatistics {
    return {
      ...
      listenerCount: Array.from(this.changeListeners.values()).reduce(
  ...
  ): Promise<Partial<IUnifiedConfig> | null> {
    try {
  ...
  private mergeConfigs(
    target: Partial<IUnifiedConfig>,
    source: Partial<IUnifiedConfig>,
  ): IUnifiedConfig {
    const merged = { ...target };
   ...
   * @returns 验证结果
   */
  private async validateFullConfig(
  ...
  private recordAccess(
    ...
    if (operation === 'read') {
      ...
      this.statistics.cacheHitRate =
        (currentHitRate + (cacheHit ? 1 : 0)) / totalReads;
    }
  }
  ...
  private updateListenerCount(): void {
    this.statistics.listenerCount = Array.from(
      this.changeListeners.values(),
 ...
 * @returns 配置管理器实例
 */
export async function createUnifiedConfigManager(
  options?: IConfigManagerOptions,
): Promise<UnifiedConfigManager> {
