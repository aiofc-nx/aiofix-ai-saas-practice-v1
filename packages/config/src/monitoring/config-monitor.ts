...
export class ConfigMonitor extends EventEmitter {
   ...
   * @returns 配置分析结果
   */
  private async analyzeConfiguration(): Promise<
    IDiagnosticReport['configuration']
  > {
    const config = await this.configManager.getConfig();
    const stats = this.configManager.getStatistics();
  ...
  private generateRecommendations(
    ...
    // 配置建议
    if (
      config.messaging.global.enableVerboseLogging &&
   ...
   * @returns 重复的配置路径
   */
  private checkDuplicateConfigs(config: IUnifiedConfig): string[] {
  ...
  private checkWeakPasswords(config: IUnifiedConfig): string[] {
    ...
    // 检查JWT密钥
    if (
      config.auth.jwt.secret === 'default-jwt-secret' ||
   ...
   * @returns 未加密的敏感信息路径
   */
  private checkUnencryptedSecrets(config: IUnifiedConfig): string[] {
   ...
   * @returns 不安全的连接路径
   */
  private checkInsecureConnections(config: IUnifiedConfig): string[] {
  ...
  private generateFixSuggestions(
    issues: IDiagnosticIssue[],
  ): IDiagnosticReport['fixSuggestions'] {
        ...
        case 'security':
          if (issue.path?.includes('jwt.secret')) {
            suggestions.push({
              issue: issue.title,
              script:
                'export AIOFIX_AUTH__JWT__SECRET=$(openssl rand -base64 32)',
 ...
 * @returns 监控器实例
 */
export async function createConfigMonitor(
  configManager: IConfigManager,
): Promise<ConfigMonitor> {
  const monitor = new ConfigMonitor(configManager);
  await monitor.initialize();
  return monitor;
}
