...
export interface IUnifiedConfigModuleAsyncOptions {
  ...
  useFactory?: (
    ...args: unknown[]
  ) => Promise<IUnifiedConfigModuleOptions> | IUnifiedConfigModuleOptions;
...
export class UnifiedConfigModule implements OnModuleInit, OnModuleDestroy {
   ...
   */
  static forRootAsync(
    options: IUnifiedConfigModuleAsyncOptions,
  ): DynamicModule {
   ...
   * @returns 动态模块
   */
  static forFeature(moduleOptions: {
    module: string;
    configPath?: string;
  }): DynamicModule {
  ...
  private static createProviders(
    options: IUnifiedConfigModuleOptions,
  ): Provider[] {
    ...
    providers.push(
      {
        ...
        useFactory: (configManager: IConfigManager) =>
          configManager.getModuleConfig('system'),
        inject: ['CONFIG_MANAGER'],
      },
      {
        provide: 'CORE_CONFIG',
        useFactory: (configManager: IConfigManager) =>
          configManager.getModuleConfig('core'),
        inject: ['CONFIG_MANAGER'],
      },
      {
        provide: 'MESSAGING_CONFIG',
        useFactory: (configManager: IConfigManager) =>
          configManager.getModuleConfig('messaging'),
        inject: ['CONFIG_MANAGER'],
      },
      {
        provide: 'AUTH_CONFIG',
        useFactory: (configManager: IConfigManager) =>
          configManager.getModuleConfig('auth'),
        inject: ['CONFIG_MANAGER'],
      },
    );
...
export const InjectConfig = (path?: string): ParameterDecorator => {
  return function (
    ...
    // 目前使用简化实现
    const token = path
      ? `CONFIG_${path.toUpperCase().replace(/\./g, '_')}`
      : 'CONFIG_MANAGER';
