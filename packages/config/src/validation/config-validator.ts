...
export class ConfigValidator implements IConfigValidator {
  ...
  async validate<T = unknown>(
    ...
    try {
      ...
      if (typeof config === 'object' && config !== null) {
        const propertyValidation = await this.validateProperties(
          schema,
  ...
  async validateUpdate(
    ...
    try {
      ...
      return {
        valid: false,
        errors: [
          `配置更新验证失败: ${error instanceof Error ? error.message : String(error)}`,
        ],
        warnings: [],
        path,
        validatedAt: new Date(),
  ...
  private async validateSystemConfig(
    ...
    if (!validEnvironments.includes(config.environment)) {
      errors.push(
        `无效的环境类型: ${config.environment}，有效值: ${validEnvironments.join(', ')}`,
      );
    }
  ...
  private async validateProperties(
    ...
    if (schema.properties) {
      for (const [propName, propSchema] of Object.entries(schema.properties)) {
        ...
        if (propValue !== undefined) {
          const propValidation = await this.validateProperty(
            propName,
            propValue,
            propSchema,
          );
    ...
    if (schema.additionalProperties === false) {
      ...
      const extraProps = Object.keys(config).filter(
  ...
  private async validateProperty(
    ...
    if (schema.type && schema.type !== actualType) {
      errors.push(
        `属性 ${name} 类型不匹配: 期望 ${schema.type}，实际 ${actualType}`,
      );
    }
    ...
    if (schema.enum && !schema.enum.includes(value)) {
      errors.push(
        `属性 ${name} 值不在允许的枚举范围内: ${schema.enum.join(', ')}`,
      );
    }
    ...
    if (typeof value === 'string') {
      ...
      if (schema.format) {
        const formatValidation = this.validateStringFormat(
          value,
          schema.format,
        );
  ...
  private async validateCustomRules(
    ...
    for (const rule of rules) {
      try {
        ...
        errors.push(
          `自定义规则 ${rule.name} 执行失败: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
  ...
  private initializeModuleSchemas(): void {
    ...
    console.log(
      '模块验证模式初始化完成:',
      Array.from(this.moduleSchemas.keys()),
    );
  }
}
