...
export const defaultConfiguration: Partial<IamConfig> = {
  ...
  setting: {
    ...
    security: {
      ...
      lockoutDuration: parseInt(
        process.env.SECURITY_LOCKOUT_DURATION || '900',
        10,
      ), // 15分钟
      sessionTimeout: parseInt(
        process.env.SECURITY_SESSION_TIMEOUT || '3600',
        10,
      ), // 1小时
      passwordMinLength: parseInt(
        process.env.SECURITY_PASSWORD_MIN_LENGTH || '8',
        10,
      ),
      requireSpecialChars:
        process.env.SECURITY_REQUIRE_SPECIAL_CHARS !== 'false',
    },
    // 功能开关
    features: {
  ...
  keycloak: {
    ...
    auth: {
      ...
      logoutUri:
        process.env.KEYCLOAK_LOGOUT_URI || 'http://localhost:3000/auth/logout',
    },
    // 用户同步配置
    sync: {
