...
export const createCacheConfig = (): CacheConfig => ({
  ...
  manager: {
    ...
    defaultStrategy:
      (process.env.CACHE_MANAGER_STRATEGY as CacheStrategy | undefined) ??
      CacheStrategy.LRU,
 ...
 * @description 获取缓存配置，与@aiofix/config集成
 * @returns {CacheConfig} 缓存配置对象
 */
export const getCacheConfig = (): CacheConfig => createCacheConfig();

/**
 * @function getCacheConfigFromAiofixConfig
 * @description 从@aiofix/config获取缓存配置
 * @param configService - @aiofix/config的ConfigService实例
 * @returns {CacheConfig} 缓存配置对象
 */
export const getCacheConfigFromAiofixConfig = (configService: any): CacheConfig => {
  const cacheConfig = configService.cacheConfig;
  return cacheConfig || createCacheConfig();
};
