...
export class SimpleCacheConfigService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `SimpleCacheConfigService初始化失败: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  private async loadConfig(): Promise<void> {
    try {
      this.config =
        await this.configManager.getModuleConfig<ISimpleCacheConfig>('cache');
    } catch (error) {
      throw new Error(
        `加载cache配置失败: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
 ...
 * 创建简化cache配置服务
 */
export async function createSimpleCacheConfigService(
  configManager: IConfigManager,
): Promise<SimpleCacheConfigService> {
