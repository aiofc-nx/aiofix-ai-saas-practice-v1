...
export interface IIsolationStrategy {
   ...
   * 应用隔离条件到查询
   */
  applyIsolation(query: any, context: DataIsolationContext): any;
...
export class TenantIsolationStrategy implements IIsolationStrategy {
  ...
  applyIsolation(query: any, context: DataIsolationContext): any {
    ...
    // 将隔离条件添加到查询中
    if (query.where) {
      query.where = { ...query.where, ...isolationConditions };
    } else {
      query.where = isolationConditions;
    }
  ...
  validateAccess(
    ...
    // 基本租户检查
    if (
      resourceContext.tenantId.toString() !== userContext.tenantId.toString()
    ) {
  ...
  getPerformanceImpact(): {
    ...
    return {
      ...
      indexRequirements: [
        'tenant_id',
        'tenant_id_organization_id',
        'tenant_id_department_id',
        'tenant_id_user_id',
      ],
    ...
    return {
      ...
      accessSuccessRate:
        this.metrics.queriesProcessed > 0
          ? (this.metrics.queriesProcessed - this.metrics.accessDenied) /
            this.metrics.queriesProcessed
          : 1,
    };
  }
  ...
  private validatePersonalAccess(
    resourceContext: DataIsolationContext,
    userContext: DataIsolationContext,
  ): boolean {
    return (
      resourceContext.userId?.toString() === userContext.userId?.toString()
    );
  }

  private validateDepartmentAccess(
    resourceContext: DataIsolationContext,
    userContext: DataIsolationContext,
  ): boolean {
    return (
      resourceContext.departmentId?.toString() ===
      userContext.departmentId?.toString()
    );
  }
  ...
  private validateOrganizationAccess(
    ...
    return (
      resourceContext.organizationId?.toString() ===
