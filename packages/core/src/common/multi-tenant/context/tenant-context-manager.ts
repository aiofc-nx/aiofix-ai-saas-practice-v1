...
export class TenantContextManager {
  ...
  static run<T>(
    tenantIdOrContext: string | ITenantContextData,
    fn: () => T,
  ): T {
    const context =
  ...
  static requireTenant(expectedTenantId: string): ITenantContextData {
    ...
    if (context.tenantId !== expectedTenantId) {
      throw new Error(
        `操作要求租户 ${expectedTenantId}，但当前租户是 ${context.tenantId}`,
      );
    }
    return context;
  }
  ...
  static createChildContext(
    updates: Partial<ITenantContextData>,
  ): ITenantContextData {
