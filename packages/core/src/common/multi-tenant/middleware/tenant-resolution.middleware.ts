...
import {
  ...
  ITenantContextData,
} from '../context/tenant-context-manager';
...
export class TenantResolutionMiddleware implements NestMiddleware {
  ...
  private extractTenantIdentifier(req: IRequest): string | null {
    // 1. 从请求头获取
    const headerTenant =
      req.headers['x-tenant-id'] || req.headers['x-tenant-code'];
  ...
  private async resolveTenantContext(
    tenantIdentifier: string,
    req: IRequest,
  ): Promise<ITenantContextData | null> {
   ...
   * 处理错误
   */
  private handleError(res: IResponse, error: any): void {
    res.status(500).json({
...
export function createTenantResolutionMiddleware(
  options: ITenantResolutionOptions = {},
