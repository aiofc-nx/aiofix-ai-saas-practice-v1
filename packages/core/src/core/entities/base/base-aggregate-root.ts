...
export abstract class BaseAggregateRoot extends BaseEntity {
   ...
   * @protected
   */
  protected applyEvent(event: BaseDomainEvent): void {
    // 子类实现具体的事件应用逻辑
  }
  ...
  public override toString(): string {
    return `${this.constructor.name}(${this.id.toString()}) - Events: ${this.getEventCount()}, Uncommitted: ${this.getUncommittedEventCount()}`;
  }
}
