...
describe('PerformanceMetricsInterface', () => {
  describe('ISystemMetrics', () => {
    it('应该正确定义系统指标接口', () => {
      ...
      expect(systemMetrics.cpuUsage).toBe(45.2);
      expect(systemMetrics.memoryUsage).toBe(67.8);
    ...
    it('应该支持可选属性', () => {
      ...
      expect(minimalSystemMetrics.cpuUsage).toBe(45.2);
    ...
    it('应该处理边界值', () => {
      ...
      expect(boundarySystemMetrics.cpuUsage).toBe(0);
      expect(boundarySystemMetrics.memoryUsage).toBe(100);
  ...
  describe('IApplicationMetrics', () => {
    it('应该正确定义应用指标接口', () => {
      ...
      expect(applicationMetrics.activeConnections).toBe(50);
      expect(applicationMetrics.queueSize).toBe(25);
      expect(applicationMetrics.cacheHitRate).toBe(0.85);
      expect(applicationMetrics.databaseConnections).toBe(10);
      expect(applicationMetrics.memoryUsage).toBe(512);
      expect(applicationMetrics.cpuUsage).toBe(30.5);
    });
    ...
    it('应该支持可选属性', () => {
      ...
      expect(minimalApplicationMetrics.memoryUsage).toBeUndefined();
      expect(minimalApplicationMetrics.cpuUsage).toBeUndefined();
    });
    ...
    it('应该处理高负载情况', () => {
      ...
      expect(highLoadMetrics.cpuUsage).toBe(95.5);
    });
  });
  ...
  describe('IPerformanceMetricsQueryOptions', () => {
    ...
    it('应该支持复杂的过滤器', () => {
      const complexFilters: IPerformanceMetricsQueryOptions = {
        ...
        endTime: new Date(),
        filters: {
          cpuUsage: { min: 20, max: 80 },
          memoryUsage: { min: 10, max: 90 },
          responseTime: { min: 0, max: 1000 },
          errorRate: { min: 0, max: 0.1 },
        },
      };

      expect(complexFilters.filters?.cpuUsage?.min).toBe(20);
      expect(complexFilters.filters?.cpuUsage?.max).toBe(80);
  ...
  describe('IPerformanceMetricsReport', () => {
    it('应该正确定义报告接口', () => {
      ...
      expect(report.metrics.system.cpuUsage.average).toBe(45.2);
  ...
  describe('边界情况', () => {
    it('应该处理极值', () => {
      const extremeMetrics: ISystemMetrics = {
        ...
        memoryUsage: 100,
        diskUsage: 100,
        networkIn: Number.MAX_SAFE_INTEGER,
        networkOut: Number.MAX_SAFE_INTEGER,
        loadAverage: [
      ...
      expect(extremeMetrics.cpuUsage).toBe(100);
      expect(extremeMetrics.memoryUsage).toBe(100);
      expect(extremeMetrics.networkIn).toBe(Number.MAX_SAFE_INTEGER);
      expect(extremeMetrics.temperature).toBe(200);
    });
    ...
    it('应该处理负数', () => {
      ...
      expect(negativeMetrics.cpuUsage).toBe(-10);
      expect(negativeMetrics.memoryUsage).toBe(-5);
