...
export interface ITenantMiddlewareConfig extends IFastifyMiddlewareConfig {
  options: {
    ...
    tenantInfoProvider?: (
      tenantId: string,
    ) => Promise<IFastifyTenantContext | null>;
  };
}
...
export class TenantMiddleware extends CoreFastifyMiddleware {
  ...
  protected override async doHandle(
    ...
    try {
      ...
      const finalTenantId = tenantId || this.getTenantOptions().defaultTenant!;
   ...
   * 执行中间件卸载
   */
  protected override async doUnregister(
  ...
  private async extractTenantId(
    ...
    if (options.tenantHeader) {
      const headerValue = request.headers[
        options.tenantHeader.toLowerCase()
      ] as string;
      if (headerValue) {
    ...
    if (options.tenantQueryParam) {
      const queryValue = (request.query as Record<string, unknown>)[
        options.tenantQueryParam
      ] as string;
      if (queryValue) {
