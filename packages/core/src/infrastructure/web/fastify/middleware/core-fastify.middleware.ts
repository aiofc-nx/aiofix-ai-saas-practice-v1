...
export class FastifyMiddlewareError extends BaseError {
  ...
  constructor(message: string, middlewareName: string, cause?: Error) {
    super(message, 'FASTIFY_MIDDLEWARE_ERROR', cause);
...
export abstract class CoreFastifyMiddleware implements IFastifyMiddleware {
  ...
  constructor(
    protected readonly _config: IFastifyMiddlewareConfig,
    ...
    try {
      const averageResponseTime =
        this._requestCount > 0
          ? this._totalExecutionTime / this._requestCount
          : 0;
   ...
   * 注册到Fastify
   */
  private async registerToFastify(
  ...
  private matchMethod(method: string): boolean {
    ...
    const methods = Array.isArray(this._config.method)
      ? this._config.method
      : [this._config.method];

    return methods.some(
      (m) =>
        m.toUpperCase() === 'ALL' || m.toUpperCase() === method.toUpperCase(),
    );
  }
  ...
  private matchPath(url: string): boolean {
    ...
    const paths = Array.isArray(this._config.path)
      ? this._config.path
      : [this._config.path];
