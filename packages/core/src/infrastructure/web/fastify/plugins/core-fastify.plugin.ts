...
import { FastifyInstance, BaseError, ILoggerService } from '../types';
import {
  IFastifyPlugin,
...
export class FastifyPluginError extends BaseError {
  ...
  constructor(message: string, pluginName: string, cause?: Error) {
    super(message, 'FASTIFY_PLUGIN_ERROR', cause);
...
export abstract class CoreFastifyPlugin implements IFastifyPlugin {
  ...
  constructor(
    protected readonly _config: IFastifyPluginConfig,
  ...
  private calculateHealthStatus(
    ...
    const failedDependencies = Object.values(dependencyStatus).filter(
      (status) => !status,
    );
