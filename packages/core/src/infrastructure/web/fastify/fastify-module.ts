 ...
 * Fastify异步模块配置接口
 */
export interface IFastifyModuleAsyncOptions
  ...
  useFactory?: (
    ...args: unknown[]
  ) => Promise<IFastifyModuleOptions> | IFastifyModuleOptions;
...
export class FastifyModule {
    ...
    return [
      ...
      },
      {
        ...
        useFactory: (
          ...
          return new CoreFastifyAdapter(config, logger || {
            info: (): void => {},
            error: (): void => {},
            warn: (): void => {},
            debug: (): void => {},
        ...
        inject: [
          FASTIFY_MODULE_OPTIONS,
          { token: 'ILoggerService', optional: true },
        ],
      },
    ];
  }
  ...
  private static createAsyncProviders(
    options: IFastifyModuleAsyncOptions,
  ): Provider[] {
      ...
      multiTenant: options.multiTenant
        ? {
            enabled:
              ...
              defaults.multiTenant?.enabled ??
              false,
            tenantHeader:
              options.multiTenant.tenantHeader ??
              defaults.multiTenant?.tenantHeader ??
              'X-Tenant-ID',
            tenantQueryParam:
              options.multiTenant.tenantQueryParam ??
              defaults.multiTenant?.tenantQueryParam ??
              'tenant',
          }
