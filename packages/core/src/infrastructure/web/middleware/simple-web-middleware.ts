...
export class SimpleWebMiddleware {
  ...
  async processRequest(requestInfo: IRequestInfo): Promise<void> {
    ...
    if (this.options.enableRequestLogging) {
      console.log(
        `[${new Date().toISOString()}] ${requestInfo.method} ${requestInfo.url}`,
      );
    }
   ...
   * 健康检查
   */
  healthCheck(): { healthy: boolean; status: string; config: any } {
    return {
      healthy: true,
      status: 'running',
      config: this.options,
   ...
   * 解析租户上下文
   */
  private resolveTenantContext(request: IRequestInfo): any {
    // 从请求头获取租户信息
