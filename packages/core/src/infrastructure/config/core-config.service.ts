...
export class CoreConfigService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `CoreConfigService初始化失败: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  private async loadConfig(): Promise<void> {
    try {
      this.config =
        await this.configManager.getModuleConfig<ICoreModuleConfig>('core');
    } catch (error) {
      throw new Error(
        `加载Core配置失败: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
 ...
 * 创建Core配置服务
 */
export async function createCoreConfigService(
  configManager: IConfigManager,
): Promise<CoreConfigService> {
