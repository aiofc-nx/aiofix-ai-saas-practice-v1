...
export interface ICoreConfigModuleAsyncOptions {
  ...
  useFactory?: (
    ...args: any[]
  ) => Promise<ICoreConfigModuleOptions> | ICoreConfigModuleOptions;
  /** 注入的依赖 */
  inject?: any[];
}
...
export class CoreConfigModule implements OnModuleInit, OnModuleDestroy {
  ...
  static forRoot(options: ICoreConfigModuleOptions = {}): DynamicModule {
    ...
    return {
      ...
      providers,
      exports: providers
        .map((p) => (typeof p === 'object' && 'provide' in p ? p.provide : p))
        .filter(Boolean),
  ...
  private static createProviders(
    options: ICoreConfigModuleOptions,
  ): Provider[] {
    const providers: Provider[] = [
      // 配置提供者
      {
        ...
        useFactory: async (configManager: IConfigManager) => {
          const config =
            await configManager.getModuleConfig<ICoreModuleConfig>('core');
  ...
  private static createAsyncProviders(
    options: ICoreConfigModuleAsyncOptions,
  ): Provider[] {
    const providers: Provider[] = [];
