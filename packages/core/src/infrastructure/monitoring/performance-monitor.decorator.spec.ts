...
describe('PerformanceMonitorDecorator', () => {
  describe('@MonitorMethod', () => {
    it('应该能够监控方法执行时间', async () => {
      class TestService {
        @MonitorMethod({
          ...
          alertThreshold: 1000,
          alertLevel: PerformanceAlertLevel.HIGH,
        })
        ...
        @MonitorMethod({
          metricType: PerformanceMetricType.RESPONSE_TIME,
          alertThreshold: 100,
          alertLevel: PerformanceAlertLevel.MEDIUM,
        })
    ...
    it('应该能够监控方法吞吐量', async () => {
      ...
      expect(results.every((result) => result === 'throughput test')).toBe(
        true,
      );
    });
  ...
  describe('@MonitorClass', () => {
    ...
    it('应该能够监控类的错误率', async () => {
      ...
      try {
        ...
        expect(error.message).toBe('Class error');
      }
    ...
    it('应该处理无效的告警级别', () => {
      class TestService {
        @MonitorMethod({
          metricType: PerformanceMetricType.RESPONSE_TIME,
          alertThreshold: 100,
          alertLevel: 'invalid_level' as any,
        })
