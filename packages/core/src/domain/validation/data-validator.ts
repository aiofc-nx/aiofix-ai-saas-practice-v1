...
import { ValidationResult } from './business-rule.entity';
...
export interface ValidationRule {
  ...
  required: boolean;
  /** 验证条件 */
  condition?: (value: any, context?: Record<string, any>) => boolean;
}
...
export class DataValidator {
    ...
    this._rules.push({
      ...
      validator: async (value) => {
        ...
        for (const [key, validator] of Object.entries(schema)) {
          const result = await validator.validate(value[key]);
          if (!result.isValid) return false;
        }
   ...
   * 同步验证（仅支持同步规则）
   */
  validateSync(value: any, context?: Record<string, any>): ValidationResult {
    const startTime = Date.now();
    const errors: string[] = [];
    const warnings: string[] = [];
