...
import { ValidationResult, RuleStatus } from './business-rule.entity';
...
export interface RuleSetValidationResult {
  ...
  /** 详细的规则验证结果 */
  readonly ruleResults: Record<string, ValidationResult>;
  /** 总执行时间（毫秒） */
...
export class RuleSet extends BaseAggregateRoot {
  constructor(
    ...
    private _type: RuleSetType,
    private _executionMode: ExecutionMode,
    private _rules: RuleReference[] = [],
  ...
  get rules(): readonly RuleReference[] {
    return [...this._rules];
  }
  ...
  updateRulePriority(ruleCode: string, newPriority: number): void {
    ...
    const updatedRule: RuleReference = {
      ...rule,
   ...
   * 获取指定规则
   */
  getRule(ruleCode: string): RuleReference | undefined {
  ...
  private async executeFailFast(
    ...
    ruleResults: Record<string, ValidationResult>,
    errors: string[],
    warnings: string[],
  ): Promise<void> {
