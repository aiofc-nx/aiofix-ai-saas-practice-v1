...
export class UserRoleAssignment extends BaseAggregateRoot {
  constructor(
    ...
    private _scope: AssignmentScope,
    private _scopeId: EntityId,
    private _assignedBy: EntityId,
    ...
    private _conditions: AuthorizationCondition[] = [],
  ...
  get conditions(): readonly AuthorizationCondition[] {
   ...
   * 获取指定类型的条件
   */
  getCondition(type: string): AuthorizationCondition | undefined {
   ...
   * 检查条件是否满足
   */
  areConditionsSatisfied(context?: Record<string, any>): boolean {
