...
import { BaseAggregateRoot } from '../entities/base/base-aggregate-root';
...
export class SecurityPolicy extends BaseAggregateRoot {
  constructor(
    ...
    private _parentPolicyId?: EntityId,
  ) {
    super(id, auditInfo);
  ...
  get configuration(): Readonly<Record<string, any>> {
   ...
   * 废弃策略
   */
  deprecate(reason: string, replacementPolicyId?: EntityId): void {
  ...
  private validatePasswordPolicyConfiguration(): void {
    ...
    if (config.minLength && (config.minLength < 6 || config.minLength > 128)) {
      throw new Error('密码最小长度必须在6-128之间');
    }

    if (config.maxAge && (config.maxAge < 1 || config.maxAge > 365)) {
      throw new Error('密码有效期必须在1-365天之间');
    }

    if (
      config.historyCount &&
      (config.historyCount < 0 || config.historyCount > 50)
    ) {
  ...
  private validateAccessControlPolicyConfiguration(): void {
    ...
    if (
      ...
      (config.sessionTimeout < 5 || config.sessionTimeout > 1440)
    ) {
    ...
    if (
      ...
      (config.maxConcurrentSessions < 1 || config.maxConcurrentSessions > 100)
    ) {
  ...
  private validateSessionManagementPolicyConfiguration(): void {
    ...
    if (
      config.idleTimeout &&
      (config.idleTimeout < 1 || config.idleTimeout > 480)
    ) {
   ...
   * 检查是否为安全降级
   */
  private isSecurityDowngrade(newConfiguration: Record<string, any>): boolean {
