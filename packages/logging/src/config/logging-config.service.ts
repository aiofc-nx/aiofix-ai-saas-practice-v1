...
export class LoggingConfigService {
  ...
  private readonly configChangeListeners: Set<
    (config: ILoggingModuleConfig) => void
  > = new Set();
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `LoggingConfigService初始化失败: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  private async loadConfig(): Promise<void> {
    try {
      this.config =
        await this.configManager.getModuleConfig<ILoggingModuleConfig>(
          'logging',
        );
    } catch (error) {
      throw new Error(
        `加载日志配置失败: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
 ...
 * 创建日志配置服务
 */
export async function createLoggingConfigService(
  configManager: IConfigManager,
): Promise<LoggingConfigService> {
