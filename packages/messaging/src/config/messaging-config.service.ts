...
export class MessagingConfigService {
  ...
  private readonly configChangeListeners: Set<
    (config: IMessagingModuleConfig) => void
  > = new Set();
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `MessagingConfigService初始化失败: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
   ...
   * @returns 处理器配置
   */
  async getHandlerConfig(
    handlerName: string,
  ): Promise<IHandlerConfig | undefined> {
    const config = await this.getConfig();
  ...
  async updateGlobalConfig(
    updates: Partial<IMessagingModuleConfig['global']>,
  ): Promise<void> {
    await this.ensureInitialized();
  ...
  async destroy(): Promise<void> {
    try {
      ...
      throw new Error(
        `MessagingConfigService销毁失败: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  private async loadConfig(): Promise<void> {
    try {
      this.config =
        await this.configManager.getModuleConfig<IMessagingModuleConfig>(
          'messaging',
        );
    } catch (error) {
      throw new Error(
        `加载messaging配置失败: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
 ...
 * @returns 配置服务实例
 */
export async function createMessagingConfigService(
  configManager: IConfigManager,
): Promise<MessagingConfigService> {
