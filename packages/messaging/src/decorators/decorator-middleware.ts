...
export function withMiddleware(
  ...middlewares: (IDecoratorMiddleware | string)[]
): MethodDecorator {
  return function (
    target: any,
...
export const LoggingMiddleware: IDecoratorMiddleware = {
  ...
  async before(context: IMiddlewareContext): Promise<void> {
    console.log(
      `[${new Date().toISOString()}] 开始执行 ${context.target.constructor.name}.${context.methodName}`,
    );
  ...
  async after(context: IMiddlewareContext, result: any): Promise<void> {
    ...
    console.log(
      `[${new Date().toISOString()}] 执行完成 ${context.target.constructor.name}.${context.methodName}`,
    );
  ...
  async onError(context: IMiddlewareContext, error: Error): Promise<void> {
    const duration = performance.now() - context.startTime;
    console.error(
      `[${new Date().toISOString()}] 执行失败 ${context.target.constructor.name}.${context.methodName}`,
    );
...
export const PerformanceMiddleware: IDecoratorMiddleware = {
  ...
  async after(context: IMiddlewareContext, result: any): Promise<void> {
    ...
    console.log(
      `性能统计 - ${context.target.constructor.name}.${context.methodName}:`,
    );
    console.log(`  执行时间: ${duration.toFixed(2)}ms`);
    console.log(
      `  内存变化: ${(memoryDiff.heapUsed / 1024 / 1024).toFixed(2)}MB`,
    );
  },
};
...
export class RetryMiddleware implements IDecoratorMiddleware {
  ...
  async onError(context: IMiddlewareContext, error: Error): Promise<void> {
    ...
    if (retryCount < this.maxRetries) {
      ...
      console.log(`重试第 ${retryCount + 1} 次，延迟 ${this.retryDelay}ms`);
      ...
      // 实际的重试逻辑需要在更高层实现
      console.log('重试逻辑需要在方法级别实现');
    } else {
      console.log(`已达到最大重试次数 ${this.maxRetries}，停止重试`);
    }
  }
}
...
export const TenantContextMiddleware: IDecoratorMiddleware = {
  ...
  async before(context: IMiddlewareContext): Promise<void> {
    ...
    console.log('设置租户上下文:', context.tenantContext);
  },
