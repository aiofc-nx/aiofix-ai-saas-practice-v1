...
export class DecoratorRegistry implements IDecoratorRegistry {
  ...
  /** 类型到处理器的索引 */
  private readonly typeIndex = new Map<
    IMessageHandlerMetadata['handlerType'],
    Set<string>
  >();
  ...
  registerHandler(metadata: IMessageHandlerMetadata): void {
    ...
    if (this.handlers.has(handlerId)) {
      throw new Error(
        `处理器已存在: ${metadata.handlerClass.name}.${metadata.methodName || 'handle'}`,
      );
    }
  ...
  private generateHandlerId(metadata: IMessageHandlerMetadata): string {
    return this.generateHandlerIdFromClass(
   ...
   * @returns 处理器ID
   */
  private generateHandlerIdFromClass(
    handlerClass: new (...args: unknown[]) => unknown,
    methodName?: string,
  ): string {
  ...
  private updateTopicIndex(
    ...
    for (const target of targets) {
      ...
      this.topicIndex.get(target)!.add(handlerId);
    }
  }
  ...
  private updateTypeIndex(
    ...
    this.typeIndex.get(metadata.handlerType)!.add(handlerId);
  }
