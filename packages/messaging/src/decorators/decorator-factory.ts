...
export class DecoratorFactory implements IDecoratorFactory {
  ...
  createSubscribe(
    ...
    return (
      ...
      const handlerClass = target.constructor as new (
        ...args: unknown[]
      ) => unknown;
  ...
  private validateTarget(target: object, decoratorName: string): void {
    ...
    if (existingMetadata) {
      throw new Error(
        `类 ${(target as Function).name} 已经应用了消息处理器装饰器`,
      );
    }
  }
  ...
  private validateMethod(
    ...
    if (existingMetadata) {
      throw new Error(
        `方法 ${target.constructor.name}.${String(propertyKey)} 已经应用了订阅装饰器`,
      );
    }
  }
  ...
  private validateQueueProcessorOptions(options: IQueueProcessorOptions): void {
    ...
    if (
      ...
      (options.maxRetries < 0 || !Number.isInteger(options.maxRetries))
    ) {
  ...
  private validateSagaOptions(options: ISagaOptions): void {
    ...
    if (
      !Array.isArray(options.triggerEvents) ||
      options.triggerEvents.length === 0
    ) {
    ...
    if (
      ...
      (options.timeout <= 0 || !Number.isInteger(options.timeout))
    ) {
